...
export const api = {
  ...
  getQASummary: async (id) => {
    ...
    return request(`${DASH_BASE}/api/ui/workflow/${id}/qa`).catch(() => null)
  },
  ...
  retestWorkflow: async (id) => {
    if (USE_DEMO) { return { workflow_id: id, status: 'RUNNING', action: 'RETEST_REQUESTED' } }
    return request(`${DASH_BASE}/api/ui/workflow/${id}/retest`, { method: 'POST' })
  },
  releaseManagerOverride: async (id, decision, reason) => {
    if (USE_DEMO) { return { workflow_id: id, status: 'COMPLETED', decision, reason } }
